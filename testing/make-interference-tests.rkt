...
; some inputs
(define b1 '((label block2861)
             (movq (int 42) (reg rax))
             (jmp conclusion)))
(define b1-las (map (λ (x) (set)) b1))
(define b2 '((label block2862)
             (movq (int 44) (reg rax))
             (jmp conclusion)))
(define b2-las (map (λ (x) (set)) b2))
(define b3 '((label block2863)
             (cmpq (var complex-if2860) (int 1))
             (jmp-if e block2861)
             (jmp block2862)))
(define b3-las (map (λ (x) (set)) b3))
(define b4 '((label block2864)
             (movq (int 0) (var complex-if2860))
             (jmp block2863)))
(define b4-las `(,(set) ,(set 'complex-if2860) ,(set 'complex-if2860)))
(define b5 '((label block2865)
             (cmpq (int 2) (int 1))
             (set e (byte-reg al))
             (movzbq (byte-reg al) (var complex-op2859))
             (cmpq (int 0) (var complex-op2859))
             (set e (byte-reg al))
             (movzbq (byte-reg al) (var complex-if2860))
             (jmp block2863)))
(define b5-las `(,(set) ,(set)  ,(set)  ,(set)  ,(set)  ,(set)  ,(set 'complex-if2860) ,(set 'complex-if2860)))
(define b6 '((label start)
             (cmpq (int 2) (int 1))
             (set l (byte-reg al))
             (movzbq (byte-reg al) (var complex-op2858))
             (cmpq (var complex-op2858) (int 1))
             (jmp-if e block2864) (jmp block2865)))
(define b6-las `(,(set) ,(set) ,(set) ,(set 'complex-op2858) ,(set) ,(set) ,(set)))

(check-equal? (length b5) (length b5-las))
(check-equal? (length b6) (length b6-las))
; expect???????
;(interference-from-live b1-las b1 '())
;(interference-from-live b2-las b2 '())
;(interference-from-live b3-las b3 '())
;(interference-from-live b4-las b4 '())
;(interference-from-live b5-las b5 '())
;(interference-from-live b6-las b6 '())
